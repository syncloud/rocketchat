...
case $1 in
start)
    bash -c "for i in `seq 1 30`; do exec ${DIR}/mongodb/bin/mongo localhost/rocketchat ${SNAP_COMMON}/config/mongo.configure.js && s=$$? && break || s=$$?; echo \"Tried $$i times. Waiting 5 secs...\"; sleep 5; done; (exit $$s)"
