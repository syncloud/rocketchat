...
var DEFAULT_CDN = 'http://artifact.syncloud.org/3rdparty'
...
function extractDownload(filePath) {
  ...
  if (filePath.substr(-4) === '.zip') {
    ...
    cp.execFile('tar', ['xf', path.resolve(filePath)], options, function (err) {
      if (err) {
...
function getDownloadSpec() {
  ...
  var downloadUrl = cdnUrl + '/phantomjs-' + downloadArch + '.tar.gz'
  var checksum = ''
